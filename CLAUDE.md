...
| 🎖️ Pontos de jogo | Minijogos (página Atividades): Dino floor(score/100) · PPT 5/vitória · **Masmorra** (`utils/dungeon.ts`): inimigos **aleatórios** do tier do pet (rookie/champion/ultimate/mega, via `STAGE_SPRITES`+`getBattleTier`; inclui Greymon/Meramon/Etemon etc.), stats escalam com a **dificuldade mensal** (`DUNGEON_DIFFICULTY`, +1 por clear, reseta todo mês pro nível 1). **Limite diário** de 3 entradas (`DUNGEON_RUNS`). **Perder custa 1 coração real** (`onLose`) → **não dá pra entrar com ≤1 coração**. **Ranking** = melhor placar (`DUNGEON_BEST`). Dropa **coraçãozinho** (`💗`, máx. 2/dia, `DUNGEON_HEART_DROPS`) — **NÃO dropa comida**. Stats do jogador escalam com o estágio (`PLAYER_STATS`). |
