...
export function CompanionHUD({ 
  ...
  const renderHearts = () => {
    const hearts = [];
    const totalHearts = Math.floor(maxHealthPoints / 2);
    
    for (let i = 0; i < totalHearts; i++) {
      const heartHP = (i + 1) * 2;
      let heartType: 'full' | 'half' | 'empty';
      
      if (healthPoints >= heartHP) {
        heartType = 'full';
      } else if (healthPoints === heartHP - 1) {
        heartType = 'half';
      } else {
        heartType = 'empty';
      }
      ...
      hearts.push(
        <div key={i} className="relative h-[22px] w-[23px] flex-shrink-0">
          ...
          {heartType === 'half' && (
            // Half heart - crop to show only left half (red)
            <div className="absolute inset-0 overflow-hidden pointer-events-none">
              <img 
                alt="" 
                className="absolute h-full left-0 max-w-none top-0 w-full" 
                src={imgHeartSprite}
                style={{ 
  ...
  // Render segmented Digivolution bar
  const renderDigivolutionBar = () => {
    return (
      <div className="flex gap-[2px]">
        {Array.from({ length: digivolutionSegmentsNeeded }, (_, i) => (
          <div
            key={i}
            className={`h-3 flex-1 transition-colors duration-300 ${
              isWin98 
                ? (i < digivolutionSegments ? 'bg-[#000080]' : 'bg-white')
                : (i < digivolutionSegments ? 'bg-gradient-to-r from-[#2bff95] to-teal-400' : 'bg-gray-600')
            }`}
            style={{ 
              minWidth: '8px',
              boxShadow: isWin98 
                ? (i < digivolutionSegments 
                  ? 'inset 1px 1px 0 #000000, inset -1px -1px 0 #1084d0' 
                  : 'inset -1px -1px 0 #ffffff, inset 1px 1px 0 #808080')
                : (i < digivolutionSegments ? '0 0 6px rgba(192, 132, 252, 0.6)' : 'none')
            }}
          />
        ))}
      </div>
    ...
    <div className={
      isGlitch 
        ...
        : isWin98 
          ...
          : 'rounded-xl p-3 relative shadow-lg'
    }
    style={
      !isGlitch && !isWin98 
        ? { backgroundColor: '#6A7282', border: '2px solid #1F2A39' }
        : undefined
    }>
      {/* Win98 Title Bar */}
      {isWin98 && (
      ...
      <div className={`flex gap-2 ${isWin98 ? 'p-2' : ''}`}>
        ...
        <div 
          className={`relative overflow-hidden p-4 flex-1 ${
            isGlitch 
              ? 'rounded border-2 border-[#00ffff]' 
              : isWin98 
        ...
        >
          ...
          {/* HP Hearts - Top Right Corner */}
          <div className="absolute top-2 right-2 flex items-center gap-1 flex-wrap z-10">
            {renderHearts()}
          </div>
          ...
          {/* Reset Timer - Bottom Left Corner */}
          <div className="absolute bottom-2 left-2 z-10">
            <span className={isWin98 ? 'text-[#00ffff] opacity-90 win98-text' : 'text-white opacity-60'} style={{ fontFamily: isWin98 ? 'Courier New, monospace' : 'monospace', fontSize: '0.625rem', textShadow: isWin98 ? '0 0 5px rgba(0, 255, 255, 0.6)' : undefined }}>
          ...
          <div className="relative w-full flex items-center justify-center" style={{ minHeight: '140px' }}>
            ...
            <div 
              ...
              style={{ 
                ...
                top: '50%',
                marginTop: '-13px',
                transition: 'transform 0.1s ease-linear, scale 0.2s ease'
              }}
            ...
            <p className={`absolute bottom-2 text-center ${isWin98 ? 'text-[#ff00ff] win98-text drop-shadow-none' : 'text-gray-800 drop-shadow-sm'}`} style={{ fontFamily: isWin98 ? 'Courier New, monospace' : 'monospace', fontSize: '0.75rem', fontWeight: 'bold', textShadow: isWin98 ? '0 0 10px rgba(255, 0, 255, 0.8), 0 0 20px rgba(0, 255, 255, 0.4)' : undefined }}>
              {currentStage}
            </p>
          </div>
        </div>
        ...
        <div 
          className={`flex flex-col items-center justify-center gap-1 px-2 py-3 ${isWin98 ? 'win98-lcd-screen' : 'rounded'}`}
          style={!isWin98 ? { backgroundColor: '#1F2A39' } : undefined}
        >
          <EnergyBar totalSteps={totalSteps} completedSteps={completedSteps} />
        </div>
      </div>

      {/* Digivolution Bar or Button - Below Companion Area */}
      <div 
        className={isWin98 ? 'mt-2 bg-gradient-to-b from-[#1a1a1a] to-[#0a0a0a] p-3 border-t border-[#00ffff]' : 'mt-2 p-3'}
        style={!isWin98 ? { backgroundColor: '#1F2A39' } : undefined}
      >
        {digivolutionSegments === digivolutionSegmentsNeeded ? (
          <button
            onClick={onDigivolve}
            className={isWin98 ? 'w-full win98-button primary py-3' : 'w-full bg-gradient-to-r from-[#2bff95] to-teal-500 hover:from-[#24e085] hover:to-teal-600 text-black py-3 px-4 rounded transition-all duration-300 transform hover:scale-105'}
          ...
          <>
            <div className="flex items-center justify-between mb-1">
              <span className={isWin98 ? 'text-[#00ffff]' : 'text-white'} style={{ fontFamily: isWin98 ? 'Courier New, monospace' : 'monospace', fontSize: '0.625rem', fontWeight: 'bold', textShadow: isWin98 ? '0 0 8px rgba(0, 255, 255, 0.8)' : undefined }}>
                DIGIVOLUTION
              </span>
              <span className={isWin98 ? 'text-[#ff00ff]' : 'text-white'} style={{ fontFamily: isWin98 ? 'Courier New, monospace' : 'monospace', fontSize: '0.625rem', textShadow: isWin98 ? '0 0 8px rgba(255, 0, 255, 0.8)' : undefined }}>
                {digivolutionSegments}/{digivolutionSegmentsNeeded} DAYS
              </span>
            </div>
            {renderDigivolutionBar()}
          </>
        )}
      </div>

      {/* Chat Box - Below Digivolution Bar */}
      <div className={isWin98 ? 'mt-2' : 'mt-2'}>
        <ChatBox 
